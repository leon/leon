...
export async function getProjects(options?: GetProjectOptions): Promise<Project[]> {
  ...
  // filters
  if (options?.topic) {
    projects = projects.filter((i) => i.topic === options.topic)
  }
  if (options?.tag) {
    projects = projects.filter((i) => i.tags?.includes(options.tag!))
  }

  // sorting
  if (options?.sortBy) {
    projects = sortBy(projects, [options.sortBy], [options.sortByOrder ?? 'asc'])
  }
